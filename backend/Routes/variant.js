...
router.post('/create',async function(req, res){
    ...
    try {
        ...
        await variant.save();
        const populatedvariant=await Variant.findById(variant._id).populate("variantTypeId");
        res.json({ success: true, message: "Variant created successfully.", data: populatedvariant });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
});
